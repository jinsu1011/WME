...
> 기준: 실제 구현 `web/frontend/src/pages`, API `제출/9반_P286_김진수_WME-API.yml`, DB `제출/9반_P286_김진수_WME-DB.dbml` — 2026-09-14 코드 대조
...
### S-05 실습 기록 — C-06
- **구성**: 시도 표(과정·회차·일시·입력 장치 `키보드 조작`/`모형 컨트롤러`·달성도 `88% · 충족`), 시드 기록에는 `예시 데이터` 배지, 빈 상태
- **동작**: 행 선택 → S-09
...
- **구성**
  ...
  - **정렬 유지 → 실습 성공** — 정렬 시작 뒤 수평(±4°) + 허용 오차 안 상태를 5초 이어서 유지하면 진행 막대(`3.8 / 5초`)가 차고
    `실습 성공` 표시(초록 테두리 깜빡임, 3D 링 깜빡임·마스크 내려옴). 벗어나면 0초부터 다시. **화면 표시이며 저장·채점과는 별개** (C-23)
  - `정렬 확정` 버튼
...
- **API**: `POST /api/attempts` → WebSocket `sample`(초당 약 25회)·`state` → `POST /api/attempts/{attempt_id}/phase`(confirmed) → `POST /api/attempts/{attempt_id}/submission`. 단계를 마칠 때마다 `PATCH /api/enrollments/{enrollment_id}/progress` — 정렬 시작 성공 시 concept·marks, 확정 시 align, 제출 시 submit
- **예외**: 서버 오류 시 안내, WebGL 불가 시 2D 현미경 뷰로 자동 대체, 확정 전에는 제출 폼 없음

### S-07a 교육용 모형 컨트롤러 연결 — C-12, C-24
- **구성**: 컨트롤러 상태 패널(모형 자세 표시, `수신 없음`/`기준 안`/`기준 밖`), `연결`·`영점 잡기`·`연결 끊기`·`키보드로 진행` 버튼, 1단계 수평 맞추기(허용 ±4.0°, `수평` 배지) · 2단계 회전 맞추기, 교육용 컨트롤러 고지
- **센서 모드 조작 (결정 15·16)**: 위치 X/Y 는 **가운데 고정**(오차 칸 회색 `고정`), 모형을 **비튼 각도만큼 웨이퍼 θ 가 그대로** 돈다(수평 잠금 없음), `±0.5°` 버튼·Q/E 는 미세조정. 수평은 실습 성공 조건에 들어간다 (C-24) · 9/15 결정 19: 영점 잡은 평행 자세 = 회전 0°, 센서 모드 회전 허용 ±3°(키보드는 ±1°)
- 키보드 모드는 기존 규칙 유지 — W/A/S/D 로 기울어져 있으면 회전 입력이 잠긴다 (C-13)
- 입력 장치를 `모형 컨트롤러`로 고르고 아직 연결하지 않았으면 `정렬 시작`이 잠긴다 ("연결하거나 키보드로 바꾸면 시작할 수 있습니다")
...
| 포트를 다른 곳에서 사용 중 | "다른 탭이나 시리얼 모니터를 닫고 새로고침" 안내 (화면을 떠나면 포트를 닫으므로 같은 탭에서는 생기지 않음) | 다른 프로그램 닫기 / 키보드 |
| 수신 없음 | 패널 `수신 없음` | 보드 확인 / 키보드 |
| 정렬 유지 중 벗어남 | 진행 막대가 0초로 돌아가고 "범위를 벗어나면 처음부터 다시 셉니다" | 다시 맞추기 |
| 수평 깨짐 (키보드 모드) | "먼저 수평을 맞추세요. 수평이 확보되어야 회전 입력이 반영됩니다." | `수평으로 되돌리기` |
...
- **API**: `POST /api/attempts` → `POST /api/attempts/{attempt_id}/submission`(`orderedIds`, `reason`) → `PATCH /api/enrollments/{enrollment_id}/progress`(제출 성공 시 단계 기록)
...
- **구성**
  ...
  - **AI 피드백** — 잘한 점 · 개선할 점 · 근거 구간 · 다음 연습 · 판단할 수 없는 것 / 실패 시: 누른 직후에는 실패 사유 문장, 다시 불러오면 `생성 실패` 카드 + "제출한 답변과 연습 기록은 그대로 저장되어 있습니다" + `다시 시도` (C-19)
  - 시도 목록, 버전 기록, `다시 연습`, 시드 기록에는 `예시 데이터` 배지
- **API**: `GET /api/attempts/{attempt_id}`, `GET /api/attempts?userId`, `GET /api/courses/{course_id}`, `GET /api/users/{user_id}`, `POST /api/attempts/{attempt_id}/feedback`, `POST /api/attempts/{attempt_id}/feedback/viewed`(학습자 본인이 피드백을 처음 볼 때 한 번. 매니저 열람은 기록하지 않음), `PATCH .../progress`(feedback 단계)
- **예외**: 없는 기록 "존재하지 않는 실습 기록입니다", 피드백 503

### S-10 교육 현황 — C-20
- **구성**: 요약 지표 4개, 학습자별 진도(단계 완료 비율), 주차별 실습 활동, 검토 필요(`needsReview`), 학습자 기록 표
- **API**: `GET /api/instructor/overview`, `GET /api/courses/{course_id}` — 집계는 화면이 계산

### S-11 학습자 목록 — C-21
- **구성**: 학습자 표(이름·소속·단계·시도 수·최근 제출·상태)
- **동작**: 행 선택 → S-12
...
> API 필드는 `9반_P286_김진수_WME-API.yml` components/schemas, **ERD(논리 설계)** 는 `9반_P286_김진수_WME-DB.dbml`,
...
| S-10·11 | 시도 수·최근 제출·검토 필요 | `InstructorRow.attemptCount` `latestAttempt` `needsReview` | `attempts` 에서 계산 | 계산값 |

### 코드 대조에서 찾은 불일치 (2026-09-14 발견 → 9/15 처리)

| # | 내용 | 조치 |
|---|---|---|
| M1 | 진도 기록 API(`PATCH /progress`)를 어떤 화면도 부르지 않음 | ✅ 9/15 FRONT 연결 — S-07·S-08·S-09 단계 완료 시 호출, 서버 저장 확인 |
| M2 | 피드백 열람 API(`POST /feedback/viewed`)를 부르지 않음 | ✅ 9/15 FRONT 연결 — S-09 학습자 본인 첫 열람 시 호출. 응답 `FeedbackViewedResponse` |
| M3 | 로그인 화면에 대응 API 없음 | 설계에 추가(`[설계]`) — 구현하지 않음 |
| M4 | S-09 판단 결과가 순서 지표를 `summary.firstPickRank` 로 읽음. API 는 `summary.scoringMetrics.firstPickRank` → server 모드에서 빈칸·`undefined/3` (C-17·C-19) | ✅ 9/15 FRONT 수정 — 화면 타입·mock·채점을 YAML 모양(`summary.scoringMetrics`)으로 맞춤. 서버·YAML 무수정. C-17·C-19 재촬영(1번째·0·3/3) |

---

## 5. 캡처 목록 (FRONT 가 만든다 → PDF UI 흐름도)

- **server 모드**(`npm run dev:server`, http://localhost:5174)에서 찍는다. 서버 데이터를 불러오는 화면에는 `실습 서버 연결` 배지가 보인다(로그인·대기방·정렬 실습 화면에는 배지가 없고, 정렬 실습 화면은 "계산 위치: 실습 서버" 로 보인다)
- 브라우저 너비 1440px 기준, 개인정보·개발 도구 없이
- 저장 위치: `제출/자료/캡처/C-xx_화면이름.png` (FRONT 예외 허용 폴더)
...
| C-13 | S-07 키보드 기울기 | 수평 깨짐 → 회전 잠김 (키보드 W/A/S/D) |
...
| C-23 | S-07 정렬 실습 | 정렬 유지 진행 막대(`4.4 / 5초`) 1장 + `실습 성공` 1장 — 키보드 모드 |
| C-24 | S-07a 모형 컨트롤러 | 실물 센서 연결 상태 — **9/15 생략**(촬영 맥에 보드 없음). PDF 에서는 C-12 와 S-07a 설명으로 대신한다 |
| C-25 | S-09 결과 (판단) | AI 피드백 성공 — `AI 채점`·`AI 생성` 배지, LLM 피드백 문장 (9/15 실제 LLM 호출 기록, 촬영 뒤 재적재로 기록 삭제) |

**최종 캡처 (9/15 FRONT)**: `제출/자료/캡처/` 25개 파일 — C-01~C-23(C-23 2장), C-25. C-07·C-17·C-19 는 M4·과정 문구 수정 뒤 재촬영
