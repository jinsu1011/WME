...
- 펌웨어 변경(재보정 `ERR,KEEP_STILL_RETRY`, 재동기 `INFO,TIMING_RESYNC`)은 커밋에 있으나 **Sensor.md 로그 없음** — PROGRESS 에 기록
- **결정 16** (FRONT 보고 A~D): A 센서 모드 수평 잠금 없음·yaw 속도 계수 미사용(서버 값 유지) / B 센서 모드 = 평행·회전 연습용 선택 입력, 채점 코드 불변, 캡처·발표 결과 화면은 키보드 기록 / C 회전 ±1° 유지 / D 센서 보정값은 화면 상수 유지
- 제출물 일관성: 화면설계 S-07(실습 성공)·S-07a(센서 모드)·예외 표·캡처 C-13 수정, **C-23·C-24 추가(24장)**. 서비스개요 FR-L07·L08·S02·예외·고려사항 10 추가
- 환경: 빈 `node_modules 3`·`.tmp` 만 든 `node_modules 2` 삭제, `./시작.sh` 집 맥북 정상. 루트 `.gitignore` 에 충돌 사본·과제 PDF 제외
- 이어서작업.md 전면 갱신(1단계 `./시작.sh`, 2단계 `./정리.sh`, 회사 순서표, 결정 6 대체·16), PROGRESS 전면 갱신, HEADER 프롬프트 회사용 재작성, FRONT·BACK 프롬프트 보정(BACK 5번 yawMode 삭제 → 시드 재적재 알림), 체크리스트 갱신
